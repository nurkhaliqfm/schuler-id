...
class SessionQuiz {
  constructor(session_id) {
    ...
    this.default_subSession = 0;

    let getSubsession = typeSoal.find(
      ({ id_main_type_soal }) =>
        id_main_type_soal == session_id[this.default_session]
    );

    let subSession = getSubsession.list_type_soal_id.split(",");

    let filteredData = dataQuiz.filter((obj) => {
      return obj.quiz_sub_subject == subSession[this.default_subSession];
    });

    finish_button.addEventListener("click", () => {
      // location.reload();
      this.CreateSession();
      while (question_num_btn.firstChild) {
        question_num_btn.removeChild(question_num_btn.firstChild);
      }
      ...
      this.NavBtnControl(current_page, filteredData);
      this.PaginationListNumber(filteredData, rows);
      this.ButtonPagination(filteredData, urlDone, urlRedirect);
    });
  ...
  CreateOption(question_id, id, value, label_option) {
    var radiobox = document.createElement("input");
    radiobox.type = "radio";
    radiobox.id = id;
    radiobox.value = value;
  ...
  DisplayList(items, rows_per_page, page, csrfName, csrfHash) {
    page--;
    let start = rows_per_page * page;
    let end = start + rows_per_page;
    let paginatedItems = items.slice(start, end);
  ...
  PaginationListNumber(items, row_per_page) {
    let page_count = Math.ceil(items.length / row_per_page);
    for (let i = 1; i < page_count + 1; i++) {
      let btn = this.BtnNumberPagination(i);
      question_num_btn.appendChild(btn);
    }
  }

  BtnNumberPagination(page) {
    let btn__side = document.createElement("div");
    btn__side.className = "question__number";
    btn__side.setAttribute("id-question", page);
    btn__side.innerHTML = page;
  ...
  ButtonPagination(items, url, urlRedirect) {
    this.SidebarStatus(current_page);
    document
      .querySelectorAll("input.form-check-input")
      .forEach((itemOption) => {
        itemOption.addEventListener("click", () => {
    ...
    next_button.addEventListener("click", () => {
      current_page = current_page + 1;
      this.DisplayList(items, rows, current_page);
      this.NavBtnControl(current_page, items);
      this.SidebarStatus(current_page);
    });
  ...
  CreateSession() {
    this.default_subSession = this.default_subSession + 1;
    return this.default_subSession;
  }
}
